...
test("keeps the V4 single-panel workstation responsive and restrained", async () => {
  ...
  assert.match(page, /const persistActiveTimelineShotScene = \(\) => \{/);
  assert.match(page, /A captured scene is authoritative[\s\S]*snapshotLocked: true,[\s\S]*dirty: false/);
  assert.match(page, /const isCurrentShot = activeShotIdRef\.current === shot\.id;\s*persistActiveTimelineShotScene\(\);[\s\S]*if \(isCurrentShot\) return;/s);
  assert.match(page, /if \(shot && shot\.id !== activeShotIdRef\.current\) \{\s*persistActiveTimelineShotScene\(\);/s);
  assert.match(page, /window\.setTimeout\(\(\) => persistActiveTimelineShotScene\(\), 180\)/);
