...
export default function Home() {
  ...
  return <SSRProvider><FluentProvider theme={{ ...webLightTheme, colorBrandBackground: "#246bfd", colorBrandBackgroundHover: "#1759d8" }} applyStylesToPortals={false} className="project-provider">
    ...
    {active ? <div className="project-workspace">
      <header className="project-workspace-header"><button onClick={() => void run(home)} disabled={busy}><ArrowLeft size={18} />项目首页</button><strong>{active.project.name}</strong><div className="project-page-switcher"><select className="project-page-select" aria-label="当前页面" value={active.page.id} disabled={busy} onChange={(event) => void run(() => open(active.project, event.target.value))}>{active.project.pages.map((page) => <option key={page.id} value={page.id}>{page.name}</option>)}</select><button aria-label="添加页面" title="添加页面" onClick={() => setAddOpen(true)}><Plus size={18} /></button><details><summary aria-label={`${active.page.name}页面菜单`}><DotsThree size={18} /></summary><div className="project-menu"><button onClick={() => setEdit({ title: "重命名页面", value: active.page.name, done: (name) => mutate(active.project, () => { active.page.name = name; }) })}>重命名页面</button><button onClick={() => void run(async () => { await flush(); const added = addProjectPage(active.project, active.page.kind, active.page.id, true); dirtyChanged(); await persist(); await open(active.project, added.id); })}>复制为新方案</button><button disabled={active.project.pages.findIndex((page) => page.id === active.page.id) === 0} onClick={() => void run(() => mutate(active.project, (project) => { const index = project.pages.findIndex((page) => page.id === active.page.id); [project.pages[index - 1], project.pages[index]] = [project.pages[index], project.pages[index - 1]]; }))}>上移</button><button disabled={active.project.pages.findIndex((page) => page.id === active.page.id) === active.project.pages.length - 1} onClick={() => void run(() => mutate(active.project, (project) => { const index = project.pages.findIndex((page) => page.id === active.page.id); [project.pages[index + 1], project.pages[index]] = [project.pages[index], project.pages[index + 1]]; }))}>下移</button><button disabled={active.project.pages.length <= 1} onClick={() => void run(() => deletePage(active.page))}>删除页面</button></div></details></div><output className={status === "保存失败" ? "failed" : ""}>{status} · 保存于此浏览器</output><button onClick={() => void run(() => exportOne(active.project))}>导出项目</button></header>
