...
export default function StudioEditor({ bridge }: { bridge: EditorBridge }) {
  ...
  useEffect(() => {
    ...
    if (cancelled) return;
    try {
      const favorites = JSON.parse(storage.getItem("poseboard.favoriteIds") ?? "[]");
      const recent = JSON.parse(storage.getItem("poseboard.recentIds") ?? "[]");
      const storedSavedPoses = readSavedPoseRecords(JSON.parse(storage.getItem("poseboard.savedPoses.v1") ?? "[]"));
      const savedProject = JSON.parse(storage.getItem("poseboard.project.v3") ?? storage.getItem("poseboard.project.v2") ?? "null") as {
      ...
      const lastSelected = storage.getItem("poseboard.lastSelectedId");
      const workspacePreferences = JSON.parse(storage.getItem("poseboard.workspace.v4") ?? "null") as { projectName?: string; contextPanelOpen?: boolean; cameraLocked?: boolean } | null;
      if (savedProject) {
        const view = bridge.view;
        const shots = (savedProject.videoTimeline as { shots?: { id: string; sceneSnapshot?: ShotSceneSnapshot }[] })?.shots;
        const selected = shots?.find((s) => s.id === view.activeShotId);
        if (selected?.sceneSnapshot) { savedProject.currentScene = structuredClone(selected.sceneSnapshot); savedProject.activeShotId = selected.id; savedProject.editor = savedProject.currentScene.editor; }
        // A newer edit from a linked page owns the shot framing. Do not restore a stale view over it.
        if (savedProject.currentScene && view.camera && view.contentVersion === bridge.contentVersion) savedProject.currentScene.camera = structuredClone(view.camera) as ShotCameraSnapshot;
        if (savedProject.currentScene && view.ratio && ratioSize[view.ratio as Ratio] && view.contentVersion === bridge.contentVersion) { savedProject.currentScene.editor.ratio = view.ratio as Ratio; savedProject.editor = savedProject.currentScene.editor; }
        if (savedProject.videoTimeline && typeof view.playhead === "number") (savedProject.videoTimeline as { playhead: number }).playhead = view.playhead;
      }
      const restoredTool = bridge.view.activeTool;
      setActiveTool(restoredTool && ["pose", "stage", "model", "camera", "perspective", "lighting", "prompt", "convert"].includes(restoredTool) ? restoredTool as ActiveTool : bridge.kind === "pose" ? "pose" : "stage");
      setTimelineOpen(bridge.view.timelineOpen ?? (bridge.kind === "animation" || bridge.kind === "storyboard"));
      setGraphOpen(bridge.view.graphOpen ?? bridge.kind === "storyboard");
      if (typeof bridge.view.zoom === "number") setZoom(clamp(bridge.view.zoom, 34, 100));
      if (typeof bridge.view.timelinePixelsPerSecond === "number") setTimelinePixelsPerSecond(bridge.view.timelinePixelsPerSecond);
      ...
      bridge.reportLoadError("此页面数据无法完整恢复，已停止自动保存以保护原内容。请导出项目备份后重新打开。");
  ...
  // Each page mounts a new editor with an immutable, project-scoped bridge.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (!persistenceReady) return;
    storage.setItem("poseboard.favoriteIds", JSON.stringify(favoriteIds));
    storage.setItem("poseboard.recentIds", JSON.stringify(recentIds.slice(0, 20)));
    storage.setItem("poseboard.savedPoses.v1", JSON.stringify(savedPoses));
    storage.setItem("poseboard.lastSelectedId", selectedPoseId);
  }, [favoriteIds, persistenceReady, recentIds, savedPoses, selectedPoseId, storage]);

  useEffect(() => {
    if (!persistenceReady) return;
    storage.setItem("poseboard.workspace.v4", JSON.stringify({ projectName, contextPanelOpen, cameraLocked }));
  }, [cameraLocked, contextPanelOpen, persistenceReady, projectName, storage]);
  ...
  const captureProjectState = () => {
    ...
    const capturedTimeline = { ...current, playhead: playheadRef.current, shots: current.shots.map((shot) => shot.id !== activeShotIdRef.current || timelinePlaying ? shot : { ...shot, sceneSnapshot: scene, snapshotLocked: true, dirty: false, aspectOverrides: { ...shot.aspectOverrides, [scene.editor.ratio]: { ratio: scene.editor.ratio, snapshot: cloneShotSceneSnapshot(scene), updatedAt: current.updatedAt } } }) };
    timelineLatestRef.current = capturedTimeline;
    storage.setItem("poseboard.project.v3", JSON.stringify({
          schemaVersion: "6.0", appVersion: "3.4.1",
          stageSettings, shotFraming, stageRecords, customShotPresets, studioGraph,
          currentScene: scene, selectedPoseId, editor: scene.editor,
          videoTimeline: capturedTimeline, animationTimeline: animationTimelineLatestRef.current,
          activeShotId: activeShotIdRef.current, timelineOpen, timelineHeight,
        }));
    const view: PageView = { activeShotId: activeShotIdRef.current, camera: scene.camera, ratio: scene.editor.ratio, zoom, playhead: playheadRef.current, timelinePixelsPerSecond, activeTool, timelineOpen, graphOpen, contextPanelOpen, cameraLocked };
    bridge.setView(view);
  };
  useEffect(() => { bridge.registerCapture(captureProjectState); return () => { bridge.registerCapture(undefined); }; });
  useEffect(() => {
    if (!persistenceReady || !modelInfo.loaded) return;
    const timer = window.setTimeout(captureProjectState, 400);
    return () => window.clearTimeout(timer);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void studioCloudRequest("PUT", buildStudioProject(), storage).then(() => { setCloudStatus("云端项目已保存"); flash("舞台与项目已保存到云端"); }).catch((error) => { setCloudStatus(error instanceof Error ? error.message : "云端保存失败，请导出 JSON 备份"); });
    }, 100);
    return () => window.clearTimeout(timer);
  ...
  const restoreCloudProject = async () => {
    ...
    try {
      const { value, etag } = await studioCloudRequest("GET", undefined, storage);
      const restored = await importStudioProject(new File([JSON.stringify(value)], "cloud-project.json", { type: "application/json" }));
      if (restored && etag) storage.setItem("poseboard.studio.etag", etag);
    ...
    >
      ...
      <header className="topbar">
        ...
        <Toolbar className="toolbar-center" aria-label={text("Canvas tools", "画板工具")}>
          ...
          <ToolbarButton className={`icon-button topbar-quick-action ${promptOpen ? "active" : ""}`} appearance="subtle" icon={<MagicWand size={19} weight={promptOpen ? "fill" : "regular"} />} aria-pressed={promptOpen} onClick={() => setPromptOpen(true)} aria-label={text("Generate", "生成")} title={text("Generate prompt", "生成提示词")}><span className="topbar-quick-action-label">{text("Generate", "生成")}</span></ToolbarButton>
          <ToolbarButton className={`icon-button topbar-quick-action ${timelinePromptOpen ? "active" : ""}`} appearance="subtle" icon={<CalendarDots size={19} weight={timelinePromptOpen ? "fill" : "regular"} />} aria-pressed={timelinePromptOpen} onClick={() => { setTimelineOpen(true); setTimelinePromptOpen(true); }} aria-label={text("Plan", "计划")} title={text("Plan shots", "计划分镜")}><span className="topbar-quick-action-label">{text("Plan", "计划")}</span></ToolbarButton>
          <ToolbarButton className="icon-button topbar-quick-action mobile-only" appearance="subtle" icon={<SidebarSimple size={19} />} aria-expanded={mobilePanel === "context"} onClick={() => setMobilePanel(mobilePanel === "context" ? null : "context")} aria-label={text("Open tool panel", "打开工具面板")} title={text("Tool panel", "工具面板")}><span className="topbar-quick-action-label">{text("Panel", "面板")}</span></ToolbarButton>
        </Toolbar>
